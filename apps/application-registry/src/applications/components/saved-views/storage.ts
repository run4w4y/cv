import {
  normalizeQueryFilterNodes,
  parseQueryFilterNodes,
  serializeQueryFilterNodes,
} from '@cv/drizzle-query-ui'
import { Option, Schema } from 'effect'
import type { SavedViewsStorage } from '../../../table-workspace/saved-view-menu'
import {
  decodeStoredJson,
  SortingStateSchema,
  TableDensitySchema,
  VisibilityStateSchema,
} from '../../../table-workspace/saved-view-schema'
import { parseCompensationDisplayCurrency } from '../../model/currency'
import {
  APPLICATION_SAVED_VIEWS_SCHEMA_VERSION,
  APPLICATION_SAVED_VIEWS_STORAGE_KEY,
  APPLICATION_WORKSPACE_STATE_SCHEMA_VERSION,
  APPLICATION_WORKSPACE_STATE_STORAGE_KEY,
  type ApplicationSavedView,
  type ApplicationSavedViewState,
} from './model'

const RawViewStateSchema = Schema.Struct({
  keyword: Schema.String,
  filters: Schema.Unknown,
  sorting: SortingStateSchema,
  columnVisibility: VisibilityStateSchema,
  density: TableDensitySchema,
  displayCurrency: Schema.String,
})

const RawSavedViewSchema = Schema.Struct({
  id: Schema.NonEmptyString,
  name: Schema.NonEmptyString,
  state: RawViewStateSchema,
  createdAt: Schema.String,
  updatedAt: Schema.String,
})

const SavedViewsPayloadSchema = Schema.Struct({
  schemaVersion: Schema.Literal(APPLICATION_SAVED_VIEWS_SCHEMA_VERSION),
  views: Schema.Array(Schema.Unknown),
})

const WorkspacePayloadSchema = Schema.Struct({
  schemaVersion: Schema.Literal(APPLICATION_WORKSPACE_STATE_SCHEMA_VERSION),
  state: RawViewStateSchema,
})

const parseViewState = (
  value: Schema.Schema.Type<typeof RawViewStateSchema>
): ApplicationSavedViewState | null => {
  const filters = normalizeQueryFilterNodes(value.filters)
  const displayCurrency = parseCompensationDisplayCurrency(
    value.displayCurrency
  )
  if (filters === undefined || displayCurrency === null) return null
  return {
    keyword: value.keyword,
    filters,
    sorting: [...value.sorting],
    columnVisibility: { ...value.columnVisibility },
    density: value.density,
    displayCurrency,
  }
}

const parseSavedView = (value: unknown): ApplicationSavedView | null => {
  const decoded = Option.getOrNull(
    Schema.decodeUnknownOption(RawSavedViewSchema)(value)
  )
  if (decoded === null || decoded.name.trim().length === 0) return null
  const state = parseViewState(decoded.state)
  return state === null
    ? null
    : {
        id: decoded.id,
        name: decoded.name,
        state,
        createdAt: decoded.createdAt,
        updatedAt: decoded.updatedAt,
      }
}

export const loadApplicationWorkspaceState = (
  storage: SavedViewsStorage | null,
  storageKey = APPLICATION_WORKSPACE_STATE_STORAGE_KEY
): ApplicationSavedViewState | null => {
  if (storage === null) return null
  const raw = storage.getItem(storageKey)
  if (raw === null) return null
  const payload = decodeStoredJson(WorkspacePayloadSchema, raw)
  return payload === null ? null : parseViewState(payload.state)
}

export const persistApplicationWorkspaceState = (
  storage: SavedViewsStorage | null,
  state: ApplicationSavedViewState,
  storageKey = APPLICATION_WORKSPACE_STATE_STORAGE_KEY
): void => {
  if (storage === null) return
  try {
    storage.setItem(
      storageKey,
      JSON.stringify({
        schemaVersion: APPLICATION_WORKSPACE_STATE_SCHEMA_VERSION,
        state: cloneApplicationViewState(state),
      })
    )
  } catch {
    // Persistence is optional and must never block table interaction.
  }
}

export const loadApplicationSavedViews = (
  storage: SavedViewsStorage | null,
  storageKey = APPLICATION_SAVED_VIEWS_STORAGE_KEY
): readonly ApplicationSavedView[] => {
  if (storage === null) return []
  const raw = storage.getItem(storageKey)
  if (raw === null) return []
  const payload = decodeStoredJson(SavedViewsPayloadSchema, raw)
  if (payload === null) return []
  return payload.views.flatMap((candidate) => {
    const view = parseSavedView(candidate)
    return view === null ? [] : [view]
  })
}

export const persistApplicationSavedViews = (
  storage: SavedViewsStorage | null,
  views: readonly ApplicationSavedView[],
  storageKey = APPLICATION_SAVED_VIEWS_STORAGE_KEY
): void => {
  if (storage === null) return
  try {
    storage.setItem(
      storageKey,
      JSON.stringify({
        schemaVersion: APPLICATION_SAVED_VIEWS_SCHEMA_VERSION,
        views,
      })
    )
  } catch {
    // A full or disabled storage must not make the table unusable.
  }
}

export const cloneApplicationViewState = (
  state: ApplicationSavedViewState
): ApplicationSavedViewState => ({
  keyword: state.keyword,
  filters:
    parseQueryFilterNodes(serializeQueryFilterNodes(state.filters) ?? '[]') ??
    [],
  sorting: state.sorting.map((entry) => ({ ...entry })),
  columnVisibility: { ...state.columnVisibility },
  density: state.density,
  displayCurrency: state.displayCurrency,
})

export const comparableApplicationViewState = (
  state: ApplicationSavedViewState
): string =>
  JSON.stringify({
    ...cloneApplicationViewState(state),
    columnVisibility: Object.fromEntries(
      Object.entries(state.columnVisibility).sort(([left], [right]) =>
        left.localeCompare(right)
      )
    ),
  })

export const describeApplicationViewState = (
  state: ApplicationSavedViewState
): string => {
  const count = countFilterConditions(state.filters)
  return `${count === 0 ? 'No filters' : `${count} ${count === 1 ? 'filter' : 'filters'}`} · ${state.density}`
}

const countFilterConditions = (
  filters: ApplicationSavedViewState['filters']
): number =>
  filters.reduce(
    (total, filter) =>
      total +
      (filter.type === 'condition'
        ? 1
        : countFilterConditions(filter.children)),
    0
  )

let fallbackId = 0
export const createApplicationViewId = (): string => {
  if (typeof crypto !== 'undefined' && 'randomUUID' in crypto) {
    return crypto.randomUUID()
  }
  fallbackId += 1
  return `application-view-${Date.now().toString(36)}-${fallbackId.toString(36)}`
}
