...
describe('preparation editor session', () => {
  ...
  test('treats a schema-valid CV as approvable without browser layout state', () => {
    const registry = AtomRegistry.make()
    ...
    const session = derive(registry, cvIdentity, head)
    expect(session.validation.valid).toBe(true)
    expect(validCvEditorDocument(session)).toEqual(validCv)
    ...
    expect(changed.canApprove).toBe(true)
  })
})
