...
export const submitPreparationReviewForRun = Effect.fn(
  'PreparationWorkflow.submitReviewForRun'
)(function* (input: SubmitPreparationReviewInput) {
  const progress = yield* PreparationProgress
  const runs = yield* SubscriptionRef.get(progress.runs)
  const run = runs.get(input.runId)
  if (run === undefined || run.status !== 'awaiting_review') {
    return yield* Effect.fail(reviewNoLongerAwaiting())
  }
  return yield* submitPreparationReview({
...
export const cancelPreparationRun = Effect.fn('PreparationWorkflow.cancelRun')(
  function* (runId: string) {
    const progress = yield* PreparationProgress
    const runs = yield* SubscriptionRef.get(progress.runs)
    const run = runs.get(runId)
    if (run?.executionId === null || run?.executionId === undefined) return
    return yield* cancelPreparation({ executionId: run.executionId, runId })
  }
)
