...
event, note, or managed application update may update the application
...
the latest event/capture without multiplying one application into several
result rows; typed count extras supply note and capture totals. The shared
...
the result into nested `counts`, `latestEvent`, and `latestCapture` values only
