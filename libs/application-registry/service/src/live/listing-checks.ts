...
import { Cause, Effect, Exit, Layer, Match } from 'effect'
...
import {
  ...
  type ScheduledListingChecksRunner as ScheduledListingChecksRunnerShape,
} from '../services/scheduled-listing-checks'
import type {
  CheckListingResult,
...
type RecordObservationInput = RecordListingObservationInput & {
  readonly claimedLeaseToken?: string
}

type ListingChecksImplementation = ListingChecksServiceShape & {
  readonly runDue: ScheduledListingChecksRunnerShape['runOnce']
}

const make = (checker: ListingAvailabilityCheckerShape) =>
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        const applied = yield* checks
          .persist({
            ...
            claimedLeaseToken: input.claimedLeaseToken,
    ...
    const checkApplication = (
      application: Application,
      input: Omit<RecordObservationInput, 'observation'>
    ) =>
    ...
    return {
      ...
      resolveAvailability: Effect.fn(
        'ListingChecksService.resolveAvailability'
      ...
      runDue: Effect.fn('ListingChecksService.runDue')(
        (input: RunDueListingChecksInput) =>
          Effect.gen(function* () {
            ...
            yield* checks.reconcileOrphanedRuns({
              failedAt: startedAt,
              staleBefore: addMinutes(startedAt, -20),
            })
            ...
            if (started === null) return { checks: [], run: null }
                      ...
                      findRequiredApplication(
                        ...
                        Effect.flatMap((application) =>
                          checkApplication(application, {
                            ...
                            requestHash: operationRequestSignature(
                              'listing_check',
                              {
                                applicationId: application.id,
                                mode: input.mode,
                            ...
                            runId,
                            claimedLeaseToken: schedule.leaseToken,
                          })
                        ),
                        Effect.matchEffect({
                          onFailure: (error) =>
                  ...
                  : registryNow.pipe(
                      Effect.flatMap((failedAt) =>
                        checks.failRun({
                          ...
                          failureMessage: Cause.pretty(exit.cause),
                          runId: run.id,
                        })
                      )
                    )
      ...
      submitFindings: Effect.fn('ListingChecksService.submitFindings')(
        (input: SubmitListingCheckFindingsInput) =>
          Effect.gen(function* () {
            ...
            return {
              archivedCount: sumBy(accepted, ({ archived }) =>
                Number(archived)
              ),
              checks: accepted.map(({ check }) => check),
              rejected,
              replayedCount: sumBy(accepted, ({ replayed }) =>
                Number(replayed)
              ),
              run,
            }
    ...
    } satisfies ListingChecksImplementation
  })

const DisabledScheduledChecker: ListingAvailabilityCheckerShape = {
  check: () =>
    Effect.die(
      new Error(
        'The HTTP registry service cannot execute scheduled listing checks.'
      )
    ),
}

export const ListingChecksServiceLive = Layer.effect(
  ListingChecksService,
  make(DisabledScheduledChecker).pipe(
    Effect.map(({ runDue: _runDue, ...service }) => service)
  )
)

export const ScheduledListingChecksRunnerLive = Layer.effect(
  ScheduledListingChecksRunner,
  ListingAvailabilityChecker.pipe(
    Effect.flatMap(make),
    Effect.map(({ runDue }) => ({ runOnce: runDue }))
  )
)
