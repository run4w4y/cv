...
import { isWithinBounds, violatesDateUnavailable } from './calendar-utils'
...
export const DateInput = React.forwardRef<HTMLDivElement, DateInputProps>(
  (
    ...
    const resolvedInputLabel = inputAriaLabel ?? ariaLabelProp ?? 'Select date'
    ...
    const selectable = React.useCallback(
      (date: Date) =>
        isWithinBounds(date, minValue, maxValue) &&
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <Calendar
            ...
            onSelect={(date) => {
              if (date && !selectable(date)) return
              setInvalid(false)
              commit(date)
              setOpen(false)
            }}
